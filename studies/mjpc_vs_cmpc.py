...
def fig_envelope(D, out):
    ...
    for p in PLANNERS:
        ...
        # RING THE POINTS THAT DID NOT ALL BRACE WITH THE COMMANDED ARM,
        # cross the targets where none did. Trunk rests are in the curve now
        # (see BP.by), so the ring is the only thing left saying that the
        # sampling planner's braced curve is mostly chest-on-table -- at most
        # of its targets in one replicate of two, at three of them in both.
        for arm in ORDER:
            allx = BP.sweep_all(D[p], arm)
            for x in sorted(allx):
                n_all = len(allx[x])
                n_ok = sum(1 for r in allx[x] if BP.arm_braced(r))
                if n_ok == n_all:
                    continue
                marked = True
                if n_ok:
                    v = _ms([r["reach_err_settled"]
                             for r in d[arm].get(x, [])])[0] * 100
...
METRICS = [
    ...
    ("trunk load", BP.trunk_load, 1.0, "N", "%.0f"),
    ("peak contact load", BP.peak_load, 1.0, "N", "%.0f"),
...
def write_table(D, out, stage="nominal", name="table_mjpc_vs_cmpc"):
    ...
    tex.append(" & ".join(texrow) + " \\\\")
    # counted over UPRIGHT rollouts: a fallen robot's torso is on the FLOOR,
    # not on the table, and counting it here reports a collapse as a brace
    row = ["%-28s" % "trunk-assisted (pooled)"]
    texrow = ["trunk-assisted (pooled)"]
    for p, a in cols:
        ...
        row.append("%16s" % ("%d/%d" % (n, len(g)) if g else "n/a"))
        texrow.append("%d/%d" % (n, len(g)) if g else "n/a")
    lines.append("".join(row) + "   --")
    tex.append(" & ".join(texrow) + " \\\\")
    ...
    open(os.path.join(out, name + ".tex"), "w").write(
        # \footnotesize and a tight \tabcolsep are IN the generated file, not
        # left to the caller: five columns of "368 $\\pm$ 320" overflow an
        # IEEEtran column by ~96 pt at \small, and a \small around an \input
        # does not shrink what is already set inside it.
